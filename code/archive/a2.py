...
def anonAssignment(taskName):
    ...
    if points == 'up':
        assiPoints.append(float(taskSplit[1].split(' ')[4]))
    else:
        assiPoints.append(float(points))
...
def anonAssignmentName(taskName):
    print(taskName)
    anonAssiName = taskName
    if anonAssiName in assignmentNames:
        return assignmentNames[anonAssiName]
    else:
        print("Assignment name: " + anonAssiName + " not found within config file")
...
def qrAnonymizationProgram(inputFile):
    ...
    print("QR file processing:")
    print("Input file: ", inputFile)    
    # annonimize input file name
    if inputFile == "-1":  # th
        return
    ...
    anonSection = anonymizeSection(section)
    ...
    print("assiname=", assiname)
    assiname = (anonAssignmentName(assiname))
    anonname += assiname + ".csv"
    anonName = "qr_"+anonname

    print("Output file:", anonName)
    ...
    # f = open("anonymization-results.txt", "w+")  # this is the anonymization-results file, doesn't really matter
...
def main():
    ...
    for unanonFile in inboxFiles:
        ...
        if inputFile == "-1":  # th
            break
        inputArray = inputFile.split("_")
        print("Input array [0]=", inputArray[0])
        if inputFile[0]=='.':
            print("Ignored file: ", inputFile)
        elif inputArray[0] == 'qr':
            print("QR call!")
                ...
                for row in reader:
                    ...
                    if counter == 0:
                        ...
                        for columnIndex in range(6, len(row)):
                            try:
                                data[counter].append(float(row[columnIndex])/assiPoints[columnIndex-6])
